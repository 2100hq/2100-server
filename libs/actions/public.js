...
module.exports = (config,libs) => {
  ...
  return user =>{
    ...
    async function getAllStakeHistoryByTime(start,end){
      assert(start,'requires start time')
      assert(end,'requires end time')

      const cacheid = ['getAllStakeHistoryByTime',start,end].join('!')

      if(queryCache.has(cacheid)){
        return queryCache.get(cacheid)
      }

      const tokens = await libs.tokens.active.list()
      const result = await Promise.reduce(tokens,async (result,token)=>{
        result[token.id] = await libs.query.stakeHistoryStatsByTime(token.id,start,end)
        return result
      },{})
