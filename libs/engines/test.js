...
const Models = {
  ...
  Types:require('../models/commands/types'),
  Stateful:require('../models/stateful'),
}

test('engine',t=>{
  let models,commands,handlers
  const tokenid = 'test'
  ...
  t.test('init models',t=>{
    models = {
      ...
      commands:Models.Commands.Model({},Models.Stateful.Model({},Cache())),
    }
    handlers = {
      transaction:Models.Commands.Types('transaction').Handler({},models)
    }
    t.end()
  })
  t.test('init wallets',async t=>{
    ...
    await models.wallets.internal.create({
      userid:tokenid,tokenid,balance:100
    })
  ...
  t.test('commands',t=>{
    ...
    t.test('init',t=>{
      commands = Commands({},{handlers,...models})
      t.ok(commands)
      t.end()
    })
    t.test('create command',async t=>{
      const cmd = await models.commands.createType('transaction',{
        tokenid,
        fromAddress:tokenid,
        fromWalletType:'internal',
        toAddress:'a',
        toWalletType:'internal',
        value:1,
        userid:'test',
      })
      // console.log(cmd)
      const result = await commands.runToDone(cmd).catch(t.end)
      console.log(result)
      const wallet = await models.wallets.internal.get('a',tokenid)
      console.log(wallet)
      t.end()
    })
  })
